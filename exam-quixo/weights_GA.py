...
class MinMaxPlayerGA(Player):
    def __init__(self, player_id: int, genotype: list[float]) -> None:
        super().__init__()
    ...
    def evaluate(self, game: 'Game') -> int:
        ...
        return evaluation if self.player_id == 0 else -evaluation
    
    def make_move(self, game: 'Game') -> tuple[tuple[int, int], Move]:
        best_move = None
        best_score = -np.inf
        for move in game.possible_moves(self.player_id):
            g = deepcopy(game)
...
def fitness(population):
    ...
    for (ind1,ind2) in tqdm(product, total=len(population)**2, desc='Fitness'):
        if ind1==ind2:
            continue
        player1 = MinMaxPlayerGA(genotype= ind1.genotype, player_id=0)
        player2 = MinMaxPlayerGA(genotype= ind2.genotype, player_id=1)       
        g = Game()
